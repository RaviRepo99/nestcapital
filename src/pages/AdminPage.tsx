              ...
              <tbody className="divide-y divide-slate-100">
                {usersList.filter((registeredUser) => registeredUser.role === 'user').map((kycUser) => (
                  <tr key={kycUser.id} className="hover:bg-slate-50/70">
                    ...
                    <td className="p-3"><button onClick={() => setSelectedKycUser(kycUser)} className="flex gap-2">{kycUser.kycDocumentImageFront ? <img src={kycUser.kycDocumentImageFront} alt={`${kycUser.fullName} document front`} className="h-14 w-24 rounded-lg border border-slate-200 object-cover" /> : <span className="text-slate-400">No front</span>}{kycUser.kycDocumentImageBack ? <img src={kycUser.kycDocumentImageBack} alt={`${kycUser.fullName} document back`} className="h-14 w-24 rounded-lg border border-slate-200 object-cover" /> : <span className="text-slate-400">No back</span>}</button></td>
      ...
      {selectedKycUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-sm p-4">
          <div className="relative w-full max-w-2xl rounded-3xl bg-white p-5 shadow-2xl text-slate-900">
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">{selectedKycUser.kycDocumentImageFront && <div><p className="mb-1 text-xs font-bold text-slate-600">Front</p><img src={selectedKycUser.kycDocumentImageFront} alt={`${selectedKycUser.fullName} identity document front`} className="max-h-[55vh] w-full rounded-2xl border border-slate-200 bg-slate-50 object-contain" /></div>}{selectedKycUser.kycDocumentImageBack && <div><p className="mb-1 text-xs font-bold text-slate-600">Back</p><img src={selectedKycUser.kycDocumentImageBack} alt={`${selectedKycUser.fullName} identity document back`} className="max-h-[55vh] w-full rounded-2xl border border-slate-200 bg-slate-50 object-contain" /></div>}</div>
